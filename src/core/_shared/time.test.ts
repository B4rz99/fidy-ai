...
it("reads an offset spelling and a bare spelling as the same UTC instant", () => {
  const withOffset = decode("2026-03-14T09:30:00-05:00");
  const asUtc = decode("2026-03-14T14:30:00Z");
  const bare = decode("2026-03-14T14:30:00");
  ...
  expect(Result.isSuccess(bare) ? DateTime.toEpochMillis(bare.success) : 0).toBe(
    Result.isSuccess(asUtc) ? DateTime.toEpochMillis(asUtc.success) : -1
  );
});
