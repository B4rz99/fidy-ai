...
const REGISTRY_SOURCE = new URL(`../${REGISTRY_PATH}`, import.meta.url);
...
const literalBody = (source: string): string => {
  ...
  if (marked === -1) {
    throw new Error(
      `${REGISTRY_PATH} has no "${LITERAL_MARKER}" declaration: the migration index is unreadable`
    );
  }
  ...
  if (close === -1) {
    throw new Error(`${REGISTRY_PATH} has an unterminated "migrations" object literal`);
  }
...
const report = (errors: readonly string[], baseRef: string): string =>
  `${REGISTRY_PATH} would lose migrations:\n\n${errors.join("\n\n")}\n\n` +
