...
- [Decide: third-party agent auth & scoping](tickets/008-decide-third-party-agent-auth.md) — per-agent hashed bearer tokens (no OAuth server at MVP; MCP is local stdio); in-chat issuance with one-time-link reveal and WhatsApp-approved CLI device flow; three operation-declared scopes (read/write/dashboard) driving tool visibility; 90-day inactivity auto-revoke + in-chat revocation; two-class per-user rate limits with numbers deferred to pricing; metadata-only audit log shared with the hosted agent.
...
- [Decide: tech stack beyond TypeScript + Effect-TS](tickets/011-decide-tech-stack.md) — Bun + Effect HttpApi (`effect/unstable/httpapi` at HEAD, not `@effect/platform`; everything derived from Schema operation definitions); Postgres-only via `@effect/sql-pg` + Effect Migrator (no Redis, no ORM); Vite/React SPA with Tailwind + shadcn/ui, Recharts widgets, TanStack Router/Query; single-process monolith on Railway (Hobby) + Railway Postgres; Resend for inbound ingest + outbound email; parsing = PDFs native to OpenAI, deterministic CSV/XLSX rows, all decoded through the canonical Effect Schema; 007's render-as-PNG endpoint amended out of MVP.
- [Decide: agent-legible response conventions for the canonical API](tickets/016-decide-agent-legible-api-conventions.md) — universal `{ data, next }` response shape via one Schema combinator (top-level `next` only); suggested operation names are canonical operation ids (identity binding, no rename); three-field suggested operations (`tool`, typed partial `args`, ≤140-char English `hint`), max 3 per response schema-enforced; handlers propose on state, an operation-derived checkpoint strikes anything the caller's scope/tier can't invoke (the aim — nothing in `next` ever fails — is unbuilt at HEAD: an entry says a call is worth making, not that it will succeed); errors mirror the success response with a closed code set, paywall errors point at new canonical op `getUpgradeUrl`.
