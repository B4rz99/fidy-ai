...
- **Kapso fits.** Developer platform on the _official_ Meta Cloud API: open-source TypeScript SDK (`@kapso/whatsapp-cloud-api`) mirroring Meta's API surface (code ports to direct-Meta), HMAC-signed webhooks with retries/idempotency, workflows-as-code, built-in **voice-note transcription**. Pro ~$25/mo for 100k messages; Meta fees passed through with no markup.
