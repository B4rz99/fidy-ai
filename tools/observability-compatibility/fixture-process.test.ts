...
const expectedSentryVersion = "10.69.0";
...
const fixture = Effect.scoped(
  Effect.gen(function* () {
    const packageManifestText = yield* Effect.promise(() => Bun.file("package.json").text());
    const packageManifest = yield* Schema.decodeEffect(Schema.fromJsonString(PackageManifest))(
      packageManifestText
    );
    ...
    return {
      runtimePinned: all(
        ...
        packageManifest.dependencies.effect === expectedEffectVersion,
        packageManifest.dependencies["@sentry/bun"] === expectedSentryVersion
      ),
