...
- `.patterns/cluster.md` — `unstable/cluster` + `ClusterWorkflowEngine`: use for production workflow composition, entities/sharding, behavioral RPC annotations, SQL message and runner storage, multi-runner/client-only topology, Snowflake identity, lifecycle, retention, transport security, and failover tests.
- `.patterns/rpc.md` — `effect/unstable/rpc`: internal typed Effect-to-Effect protocols, schema-defined operations/groups, client scopes, transport/serialization bounds, request identity, interruption, errors, and Cluster composition. HttpApi remains the public API boundary.
