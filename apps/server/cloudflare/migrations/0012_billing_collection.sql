...
-- A support-confirmed absence of a charge releases admission; it is never settlement evidence.
-- Privileged operators must supply attributable Wompi case evidence for this exact sent arm.
CREATE TABLE billing_no_charge_confirmations (
  attempt_id TEXT PRIMARY KEY REFERENCES billing_attempts(id),
  wompi_reference TEXT NOT NULL,
  wompi_environment TEXT NOT NULL,
  provider_case_id TEXT NOT NULL CHECK (length(trim(provider_case_id)) BETWEEN 1 AND 128),
  operator_id TEXT NOT NULL CHECK (length(trim(operator_id)) BETWEEN 1 AND 128),
  confirmed_at_ms INTEGER NOT NULL CHECK (confirmed_at_ms > 0)
) STRICT;
CREATE TRIGGER billing_no_charge_requires_sent_arm BEFORE INSERT ON billing_no_charge_confirmations
WHEN NOT EXISTS (SELECT 1 FROM billing_attempts AS a
  JOIN billing_collection_arms AS arm ON arm.attempt_id = a.id
  WHERE a.id = NEW.attempt_id AND a.wompi_reference = NEW.wompi_reference
    AND a.wompi_environment = NEW.wompi_environment AND a.status <> 'succeeded'
    AND arm.state = 'sent')
BEGIN SELECT RAISE(ABORT, 'billing_no_charge_unverified_arm'); END;
CREATE TRIGGER billing_no_charge_immutable_update BEFORE UPDATE ON billing_no_charge_confirmations
BEGIN SELECT RAISE(ABORT, 'billing_no_charge_immutable'); END;
CREATE TRIGGER billing_no_charge_immutable_delete BEFORE DELETE ON billing_no_charge_confirmations
BEGIN SELECT RAISE(ABORT, 'billing_no_charge_immutable'); END;
-- The admission decision and BillingAttempt insertion share the D1 atomic unit.
...
WHEN EXISTS (SELECT 1 FROM billing_attempts AS prior
  WHERE prior.user_id = NEW.user_id AND prior.status <> 'succeeded'
    AND NOT EXISTS (SELECT 1 FROM billing_no_charge_confirmations AS clear
      WHERE clear.attempt_id = prior.id))
...
-- A confirmation followed by verified approval needs human investigation of any later collection.
CREATE TABLE billing_recovery_reviews (
  attempt_id TEXT PRIMARY KEY REFERENCES billing_attempts(id),
