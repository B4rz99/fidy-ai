...
COPY --chown=bun:bun scripts/prepare-sentry-release.ts scripts/provision-runtime-role.ts scripts/migrate.ts scripts/smoke-sentry-deployment.ts ./scripts/
RUN bun run build:production && cp "$(bun -e 'console.log(require("@sentry/cli").SentryCli.getPath())')" dist/commands/sentry-cli
