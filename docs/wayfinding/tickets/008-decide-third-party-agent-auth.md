...
### 3. Scopes: three coarse scopes, declared in the operation definition
...
- **Each canonical operation's Effect Schema operation definition declares its required scope**; HTTP authz middleware, MCP tool visibility, and CLI commands all derive from it — an agent only sees the tools its token can call, so enforcement cannot drift from the surface.
...
- **Two operation classes declared in the same operation definition as the scope**: cheap (CRUD/queries — generous token bucket, order of 60 req/min with burst) and expensive (LLM-backed extraction, rendering — tight daily caps). Standard `429` + `Retry-After`; small per-user concurrency cap.
