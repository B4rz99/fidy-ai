...
their WhatsAppIdentity, their RecoveryEmailCredential, or a tracked SupportRecoveryCase backed by
...
User may approve the pairing through their verified WhatsApp association, prove the one verified
recovery email through a direct first-party form, or give support the public pairing reference plus
the pre-issued BackupRecoveryCode. The private verifier never leaves the browser, and no public
reference or second-channel proof can establish a session by itself. Pairing expires after ten
minutes, succeeds once, and puts no private verifier, session bearer, or recovery proof in chat,
Transcript, model context, URLs, provider payloads, logs, analytics, or recoverable storage.
...
- Email recovery verifies a short-lived purpose-bound proof for the existing
  RecoveryEmailCredential and the live pairing private verifier before approval.
...
create a User, replace RecoveryEmailCredential, or modify WhatsAppIdentity. The browser polls with
...
- **Recovery-specific WebSession:** rejected because it would duplicate session issuance,
