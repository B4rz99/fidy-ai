...
The **phase boundary** is the gap between two phases, and it is the only place this decision belongs. Mid-phase there is no decision to make — continue, or split the work that's left into Herdr Pi workers. Compacting mid-phase makes the agent lose the thread.
...
| **Herdr Pi worker** | Send a tightly-scoped task to its own Herdr pane and get a report back. |
...
- swapping to a **new harness** (for example, Claude → Pi),
...
**4. Can the task be done AFK?** Is it scoped tightly enough to run with you away from the keyboard, with no steering? Then send it to a **Herdr Pi worker** and leave this session untouched. Check `HERDR_ENV=1` first and follow `/herdr`'s worker procedure; there is no API or in-process fallback. Automated review is the standard case: the worker reads the diff and reports, and you aren't needed while it does.
