...
The browser-login shell owns the unbound challenge and its lifecycle. Anonymous start admission and insertion are one database transaction. Approval is a canonical mutation whose caller-supplied input is only the public code; resolved authority supplies the stable UserId. The hosted execution boundary admits that mutation only when its authority root is a verified WhatsApp association and after exact confirmation. One user-scoped database transaction serializes approval for the User, binds the pending challenge, supersedes that User's older Ready pairing, and appends canonical audit evidence. No browser, PAT, MCP, CLI, or anonymous boundary can authorize that transition.
