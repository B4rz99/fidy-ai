...
HATEOAS-style affordances, revived for agents ([htmx essay](https://htmx.org/essays/hateoas/)): LLM agents are the first API clients that can act on next-action links they weren't programmed for, and third-party agents ([auth & scoping decision](008-decide-third-party-agent-auth.md)) can't be prompt-engineered — the response body is the only channel for guiding them. Decide the canonical API's agent-legible response conventions:

1. **Response envelope** — shape of a shared `data` + `next` (affordances) envelope, expressed as an Effect Schema combinator over the [HttpApi contracts](011-decide-tech-stack.md). How do affordance names bind to tool names so MCP/CLI toolkits and in-response affordances stay in sync?
2. **Affordance conditioning** — which affordances appear per response, conditioned on resource state, token scope (008: read/write/dashboard), and tier ([free/Pro boundary](010-decide-pricing-and-free-tier.md)). Discipline: few high-value next-actions, not exhaustive menus — every affordance is tokens in an agent's context.
3. **Error shape** — `action_required` errors with reason + next steps + hint (vs bare 4xx), including the paywall case: a free-tier agent crossing the history boundary should receive a self-describing upgrade affordance, not a bare 402.
...
- **Every canonical operation's success response is `{ data, next }`**, applied as a single
  Effect Schema combinator at the contract layer (`withAffordances(DataSchema)`) — no per-operation
  opt-out, so the shape cannot drift across the derived HTTP/MCP/CLI/toolkit surfaces (011's parity
  machinery extends to the envelope).
- **`next` appears only at the top level of a response, never nested per-item** in list responses —
  affordances describe the response as a whole (referencing item ids where needed), not each row.
  Empty `next: []` costs ~4 tokens; no conditional envelope machinery.

### 2. Name binding: the operation id is the name, everywhere

- An affordance's `tool` field **is the canonical operation id** (`createBudget`, `listTransactions`).
...
- The CLI (its own naming grammar) derives command names mechanically from the operation id and
  **echoes the operation id in help/output**, so a CLI-driving agent resolves `createBudget` → command
  deterministically.
- Rejected: a separate semantic affordance vocabulary (HATEOAS-style `rel`s mapped onto operations) —
...
### 4. Conditioning: handlers propose, a contract-derived checkpoint disposes
...
### 5. Errors mirror the envelope; the upgrade link is a canonical operation
...
- **`next` reuses the same affordance type and callability invariant** — which forces the upsell to
...
- `scope_missing` carries **no affordance** — minting/broadening a token happens in WhatsApp chat
...
- **Hard cap of 3 affordances per response** in the schema (`maxItems(3)`) — a handler proposing more
