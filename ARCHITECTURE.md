...
### Core slices cross only through published references

A core slice may import ownerless **value types** from `core/_shared` — `Money`, interpretation
context, canonical operation identity, date ranges, and generic helpers. A value with a natural
slice owner lives in that slice's narrow `reference.ts` interface instead: Identity publishes
`UserId` and `E164PhoneNumber`, Categories publishes `CategoryId`, Tokens publishes
`AgentTokenId`, and Insights publishes `InsightKind`.

A slice earns `reference.ts` only when another core slice genuinely needs to name or persist one of
its stable ids or kind codes. The file publishes only those stable values and their interface
comments — never a model, rule, error, repository, taxonomy, or other implementation detail. A
sibling core import may cross only that direct reference interface; shell may import any owning core
interface it needs.

Where a decision needs another slice's data, the shell still loads both and passes plain values in:
...
RecurringSeries, reports, and billing all need exactly the same monetary meaning. Ownerless values
remain shared; a slice does not manufacture an owner or an empty reference interface for them.
Owning slices add contextual rules — for example, Money permits zero while a Transaction requires a
positive amount — but they do not redefine the value.

**Core decides; it does not gather.** A sibling reference carries a stable value's meaning, not the
sibling's data. Importing the sibling's model, rules, errors, or implementation would claim to know
how to obtain or decide over that data, which is shell's job. This also resolves the suggested
operation checkpoint — a pure function over `scope` and `tier` as plain arguments, so
`core/_shared` depends on no slice.
...
| Core slices cross only through references and shared values | Free cross-slice imports                                                   | Core would start gathering or depending on another slice's implementation, which is shell's job                                                                                                                                                                                                                                   |
