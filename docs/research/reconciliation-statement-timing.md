...
> **Later decision:** the prototype discussed in [#433](https://github.com/B4rz99/fidy-ai/issues/433) removed model judgment from the MVP. Timing findings and boundaries remain authoritative; references below to model ranking are superseded by deterministic linking or User review.
...
Start durable Reconciliation work **when either an eligible notification-email Transaction or a statement-line Transaction and its SourceAttestation commit**. Insert a bounded outbox intent in the same User-scoped D1 atomic unit as that capture. A uniqueness constraint or equivalent idempotent claim must make either arrival order safe without duplicating pair decisions.
...
The canonical `TransactionExtraction` carries an exact UTC `occurredAt` value describing when the Money moved, while receipt and processing times have separate lifecycle uses (see `core/transactions/model.ts`). Exact schema precision does not prove that every source supplied an exact purchase time; the retained interpretation revision remains necessary. The timing rule deliberately projects the value to a calendar date rather than depending on sub-day precision. A future Cloudflare email adapter must preserve this distinction.
...
The future D1 ingestion adapter must call the canonical statement-capture operation for accepted rows and for NeedsReviewItem resolution, and must call the source-specific capture operation for notification evidence. Those calls must remain inside the caller-owned D1 atomic unit; the current server seam intentionally fails closed until that adapter exists.
