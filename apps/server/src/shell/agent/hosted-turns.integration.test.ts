...
import { clusterMessagesTable } from "~/shell/durable-tables";
...
const mailbox = Effect.gen(function* () {
  ...
  return yield* Schema.decodeUnknownEffect(
    ...
    yield* sql`SELECT processed FROM fidy_durable.${sql(clusterMessagesTable)} WHERE entity_type = ${HostedTurns.type} AND kind = 0`
  );
});
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            yield* sql`SELECT request_id FROM fidy_durable.${sql(clusterMessagesTable)}
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const sql = yield* MigrationSqlClient;
          const messages =
            yield* sql`SELECT payload, headers FROM fidy_durable.${sql(clusterMessagesTable)} WHERE entity_type = ${HostedTurns.type}`;
