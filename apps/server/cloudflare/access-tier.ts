 ...
 *
 * The canonical decision is `~/core/access-tier/operations.ts:decideAccessTier`, and the complete
 * standing projection and Paywall behavior are #712's; this predicate is the bounded SQL projection
...
export const activeProUserSql = `(EXISTS (SELECT 1 FROM trial_periods AS trial
    WHERE trial.user_id = ? AND trial.ends_at_ms > ?)
