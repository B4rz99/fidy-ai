...
A shell-only PAT lifecycle process opens one User-scoped PostgreSQL transaction and invokes transaction-aware operations published by the PAT and Consent owners. Manual issuance and approved PATPairing commit the PAT state and grant ConsentRecord together. User revocation, revoke-all, approved-but-unclaimed expiry, and 90-day inactivity revocation commit the PAT state and symmetric revocation ConsentRecord together. A failure commits neither, and retries append no duplicate evidence.

The coordinating process never writes owner tables directly and performs no browser, messaging, model, or other provider call inside the transaction. Consent evidence records its honest origin: authenticated web decision, provider-qualified chat decision for revocation, or automatic policy. Authentication denies an idle-expired or revoked PAT immediately; scheduled work subsequently records automatic expiry through the same atomic owner operations.
